...
object Database {
  ...
  object Connection {
    ...
    def transactor(config: DatabaseConfig): ZManaged[Blocking, Throwable, Transactor[Task]] =
      for {
    ...
    val live: ZLayer[DbConfigEnv with Blocking, Throwable, TransactorEnv] = ZLayer.fromServiceManaged[DatabaseConfig, Blocking, Throwable, Transactor[Task]](cfg => transactor(cfg))
  }
}
